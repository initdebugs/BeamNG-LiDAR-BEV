...
class BeamNgWorker(QObject):
    ...
    def attach_to_player(self) -> None:
        ...
        try:
            ...
            for index, mount in enumerate(geometry.mounts.values()):
                self.status_changed.emit(
                    "ATTACHING", f"Attaching {mount.name} LiDAR ({index + 1}/4)"
                )
                sensor = Lidar(
                    ...
                    vertical_angle=LIDAR_VERTICAL_FOV_DEG,
                    frequency=LIDAR_UPDATE_HZ,
                    # Per-mount, not global: the front unit reaches much
                    # further on a narrow, dense sweep. See SensorMount.
                ...
                if index == 0:
                    self._verify_mount_height(sensor, vehicle)
        ...
        self.status_changed.emit(
            "STREAMING", f"Four LiDAR sensors active on {player_vid}"
        )
    ...
    def _poll_once(self) -> None:
        ...
        try:
            state = self._get_vehicle_state()
            self._last_speed = float(
                np.linalg.norm(vec3(state.get("vel", (0.0, 0.0, 0.0))))
            )
            ...
            if point_chunks:
                ...
                road_mask = classify_road_points(
                    colours,
                    heights,
    ...
    @staticmethod
    def _verify_mount_height(sensor: Lidar, vehicle: Vehicle) -> None:
        """
        Confirm the simulator placed the sensor where we asked.
        ...
        """
        try:
            world_z = float(sensor.get_position()[2])
            bbox_z = min(float(vec3(p)[2]) for p in vehicle.get_bbox().values())
            delta = world_z - bbox_z
        except Exception:
            LOGGER.debug("Could not verify sensor mount height", exc_info=True)
            return
        LOGGER.info(
            "Mount check: sensor sits %.3f m above the bbox bottom (want %.2f)",
            delta,
            SENSOR_HEIGHT_ABOVE_GROUND_M,
        )
        if abs(delta - SENSOR_HEIGHT_ABOVE_GROUND_M) > 0.15:
            LOGGER.warning(
                "Sensor mount height is off by %.3f m. Below ground means no "
                "downward rays and a collapsed horizontal sweep.",
                delta - SENSOR_HEIGHT_ABOVE_GROUND_M,
            )
